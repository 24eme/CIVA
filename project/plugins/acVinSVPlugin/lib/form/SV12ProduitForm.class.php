...
class SV12ProduitForm extends acCouchdbObjectForm
{
    ...
    public function configure() {
        $this->setWidget('superficie_recolte', new bsWidgetFormInputFloat(array(), ['class' => 'form-control text-right input-float input-sm', 'disabled' => $this->getObject()->isRebeche()]));
        $this->setValidator('superficie_recolte', new sfValidatorNumber(array('required' => false)));

        $this->setWidget('quantite_recolte', new bsWidgetFormInputInteger(array(), array('class' => 'form-control text-right input-integer input-sm input_quantite')));
        $this->setValidator('quantite_recolte', new sfValidatorInteger(array('required' => false)));

        $this->setWidget('taux_extraction', new bsWidgetFormInputFloat(array(), array('class' => 'form-control text-right input-float input-sm input_taux_extraction', 'readonly' => 'readonly', 'tabindex' => -1)));
        ...
        $this->setWidget('volume_revendique', new bsWidgetFormInputFloat(array(), ['placeholder' => '', 'class' => 'form-control text-right input-float input-sm input_volume_revendique', 'disabled' => $this->getObject()->isRebeche()]));
