...
class VracTousView extends acCouchdbView
{
    ...
    public function findSortedByDeclarants(array $tiers, $campagne = null, $statut = null, $type = null, $role = null, $commercial = null, $temporalite = null) {
        ...
        foreach ($result as $key => $vrac) {
            ...
            if($item->type_creation == VracClient::TYPE_CREATION_PAPIER && in_array($item->statut, array(Vrac::STATUT_CREE)) && !sfContext::getInstance()->getUser()->hasCredential(CompteSecurityUser::CREDENTIAL_ADMIN)) {
                unset($result[$key]);
                continue;
            }
        }
