...
class sfCouchdbBuildViewTask extends sfBaseTask {
...
The [sfCouchdbBuildViewTask|INFO] task does things.
Call it with:

  [php symfony sfCouchdbBuildViewTask|INFO]
EOF;
    }
    ...
    protected function execute($arguments = array(), $options = array()) {
    	...
    	foreach($designs as $design) {
    		sfCouchdbManager::getClient()->storeDoc($design);
    		foreach($design->views as $view => $content) {
    			echo sfCouchdbManager::getClient()->getDatabaseUri().'/'.$design->_id."/_view/".$view."\n";
    		}
    	}
    }
    ...
    protected function getDesign($name) {
    	...
    	$design = sfCouchdbManager::getClient()->retrieveDocumentById($id, sfCouchdbClient::HYDRATE_JSON);
    	if (!$design) {
    		$design = new stdClass();
