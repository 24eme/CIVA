...
echo 'type;annee;"CVI acheteur";"Nom acheteur";"CVI récoltant";"Nom récoltant";Appellation;Lieu;Cépage;VTSGN;Dénomination;"Superficie livrée";"Qté livrée en kg";"Volume livré";"Volume à détruire";"Dont VCI";"Volume revendiqué";"Date de validation";"Hash produit";"Document ID"' > data/sv.utf8.csv
for (( i=2017; i <= $(date +"%Y"); i++ ));
do
    bash bin/export_svs_csv.sh $i | grep -v "Hash produit" >> data/sv.utf8.csv
done
