...
echo "campagne;num tiers;num stock;cvi;civaba;type de ds;appellation;lieu;cepage;volume total;volume normal;volume vt;volume sgn"
join -t ";" $TMPDIR/tiers_db2_$KEYUNICITE.sorted.csv $TMPDIR/ds_bi_$KEYUNICITE.sorted.csv -1 3 -2 2 -a 2 | awk -F ";" '{ cvi=""; civaba=""; if($1 ~ /^C/) { civaba=substr($1,2) } else { cvi=$1 } print $4 ";" $2 ";" $3 ";" cvi ";" civaba ";" $5 ";" $6 ";" $7 ";" $8 ";" $9 ";" $10 ";" $11 ";" $12 }' | sort -nt ";" -k 1,2
