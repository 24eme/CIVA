...
cat $TMPDIR/ds_bi_$KEYUNICITE.csv | grep -v "^#" | awk -F ";" '{ appellation=$13 " " $15;lieu=$14; if(!lieu && $17) {lieu=$17} gsub(/[\ ]+$/, "", appellation); gsub(/[\ ]+/, " ", appellation); type_ds="P"; if($4 ~ /^C/) {type_ds="N"; } print substr($1,0,4) ";" $4 ";" type_ds ";" appellation ";" lieu ";" $16 ";" $18 ";" $19 ";" $20 ";" $21 ";" $2 }' | grep -Ev ";(P|N);(mouts|rebeches|lies|dplc|);" | sed 's/\"//g' | sort -t ";" -k 1,1 >> $TMPDIR/ds_bi_$KEYUNICITE.sorted.csv

echo "campagne;num tiers;num stock;identifiant;cvi;civaba;type tiers;type de ds;appellation;lieu;cepage;volume total;volume normal;volume vt;volume sgn;id doc"
join -t ";" $TMPDIR/tiers_db2_$KEYUNICITE.sorted.csv $TMPDIR/ds_bi_$KEYUNICITE.sorted.csv -1 3 -2 2 -a 2 | awk -F ";" '{ cvi=""; civaba=""; if($1 ~ /^C/) { civaba=substr($1,2) } else { cvi=$1 } print $5 ";" $2 ";" $3 ";" $1 ";" cvi ";" civaba ";" $4 ";" $6 ";" $7 ";" $8 ";" $9 ";" $10 ";" $11 ";" $12 ";" $13 ";" $14 }' | sort -nt ";" -k 1,2
