...
join -t ";" -a 1 /tmp/recoltant /tmp/compte_droit_dr | while read line; do CVI=$(echo -n $line | cut -d ";" -f 1); curl -s $COUCHDBDOMAIN:$COUCHDBPORT/$COUCHDBBASE/DR-$CVI-$ANNEE | grep "not_found" && echo $line; done | grep -v not_found
