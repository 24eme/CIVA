...
bash bin/export_drs_csv.sh $ANNEE | grep -v '"CVI acheteur"' | grep -v "mentionVT" | grep -v "mentionSGN" | cut -d ";" -f -18 | grep -Ei "(séléction|selection|grain| nobles|vendange|tardive|sgn|vt|v\.t|s\.g\.n)"
