...
DESCRIPTION="Synthese de l'export JSON douane des SV11 de la campagne $DR_CAMPAGNE"

. bin/task_start.inc

mkdir -m 777 $TASK_DIR/tmp > /dev/null

JSON_FILE=tmp/export_sv11_"$DR_CAMPAGNE"_$(date +%Y%m%d%H%M%S).json
EXPORT_FILE=tmp/synthese_sv11_"$DR_CAMPAGNE"_$(date +%Y%m%d%H%M%S).csv
