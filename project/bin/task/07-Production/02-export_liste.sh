...
bash bin/export_svs_csv.sh "$DR_CAMPAGNE" | tail -n +2 | awk -F';' -v CAMPAGNE="$DR_CAMPAGNE" '{v=""; if($17) {v="VALIDEE"} ; print CAMPAGNE ";" $1 ";" $2 ";" v ";" $17 }' | uniq | sort > "$TASK_DIR/$EXPORT_FILE.sv"
