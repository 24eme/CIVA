...
class ExportDS
{
	...
	public function __construct(Export $export, $function_get_partial, $debug = false) {
        ...
        $this->_ids = array();
        if($export->exist('dss')) {
		  $this->_ids = $this->findIds($export->dss);
        }
        $this->_file_export_ds_pdfs = $this->generateFileExportDSPdfs($function_get_partial);
		$this->_file_dir = sfConfig::get('sf_web_dir') . '/mise_a_disposition/'.$this->_export->cle.'/declarations_de_stocks';
		$this->createFolder();
	}
    ...
    public function exportById($id) {
        ...
        if (!array_key_exists($id, $this->_file_export_ds_pdfs)) {
            throw new sfException("This ds not existing in this export : ".$id);
        }

        $file_export_ds = $this->_file_export_ds_pdfs[$id];
        
        if (!$file_export_ds->isExported()) {
            $file_export_ds->export();
            $exported = $file_export_ds->isExported();
    ...
    public function publicationById($id) {
        ...
        $file_export_ds = $this->_file_export_ds_pdfs[$id];
        $campagne = $file_export_ds->getDS()->annee;

        if ($this->getHashMd5($campagne) && $this->getHashMd5($campagne) == $this->getHashMd5FileFromFile($campagne)) {
            return;
        }

        $path = $this->_file_dir.'/'.$file_export_ds->getDS()->annee.'/DS-'.$file_export_ds->getDS()->identifiant.'-'.$file_export_ds->getDS()->periode.'.pdf';
        $this->mkdirUnlessFolder($this->_file_dir.'/'.$file_export_ds->getDS()->annee);
        ...
        if (!($file_export_ds->getDS()->validee && $file_export_ds->getDS()->modifiee)) {
            throw new sfException("This ds in not valid or has been imported from db2 : ".$id);
        }

        link($file_export_ds->getPath(), $path);
    ...
    public function getHashMd5($campagne) {
        if (!array_key_exists($campagne, $this->_hash_md5)) {
            $hash_md5 = '';
            foreach($this->_file_export_ds_pdfs as $id => $file_export_ds_pdf) {
                if ($file_export_ds_pdf->getDS()->annee == $campagne) {
                    $hash_md5 .=  $id.$file_export_ds_pdf->getDS()->_rev;
                }
            }
    ...
    protected function generateFileExportDSPdfs($function_get_partial) {
        $file_export_ds_pdfs = array();
        foreach ($this->getIds() as $id) {
            try {
                $file_export_ds_pdfs[$id] = new FileExportDSPdf($id, $function_get_partial);
                if (!in_array($file_export_ds_pdfs[$id]->getDS()->annee, $this->_campagnes)) {
                    $this->_campagnes[] = $file_export_ds_pdfs[$id]->getDS()->annee;
                }
            } catch (Exception $e) {
                if ($this->_debug) {
	...
	protected function findIds($dss) {
		$ids = $dss->ids->toArray();

        foreach($dss->views as $view) {
