<ul id="btn_appelation" class="btn_prev_suiv clearfix">
    <?php $margin = "190px"; ?>
    <li class="prec">
        <a href="<?php echo url_for('recolte_noeud_precedent', array('hash' => $produit->getMention()->getHash())) ?>" class="btn_recolte_can_be_inactif" <?php if (isset($is_recap) && $is_recap): ?>onclick="document.getElementById('principal').submit(); return false;"<?php endif; ?>>
    ...
    <li class="suiv">
        ...
        <a href="" onclick="if (valider_can_submit()) { document.getElementById('principal').submit(); } return false;">
            <?php else: ?>
            <a href="<?php echo url_for('recolte_recapitulatif', array("hash" => $produit->getLieu()->getHash())) ?>" class="btn_recolte_can_be_inactif">
                <?php endif; ?>
