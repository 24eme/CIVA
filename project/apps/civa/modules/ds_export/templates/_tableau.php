...
<?php if(!array_key_exists('no_header', $tableau) || !$tableau['no_header']): ?>
  <tr>
    ...
    <th style="text-align: left; font-weight: bold; width: <?php echo round(318/count($tableau['colonnes'])) ?>px; border: 1px solid black;">&nbsp;<?php echo $libelle ?></th>
    <?php endforeach; ?>
...
<?php foreach($tableau['produits'] as $produit): ?>
  <tr>
    ...
    <?php if($colonne["rowspan"] > 0): ?>
      <td style="text-align: left; border: 1px solid black; width: <?php echo round(318/count($tableau['colonnes'])) ?>px; <?php if(is_null($colonne['libelle'])): ?>background-color: #bbb;<?php endif; ?>" rowspan="<?php echo $colonne["rowspan"] ?>">&nbsp;<?php echo truncate_text($colonne['libelle'], round(50 / count($tableau['colonnes'])), "<small>…</small>", false) ?></td>
    <?php endif; ?>
    <?php endforeach; ?>
    <td style="border: 1px solid black; width: 106px; <?php if(is_null($produit["normal"])): ?>background-color: #bbb;<?php endif; ?>"><?php echoVolume($produit["normal"]) ?></td>
    <td style="border: 1px solid black; width: 106px;<?php if(is_null($produit["vt"])): ?>background-color: #bbb;<?php endif; ?>"><?php echoVolume($produit["vt"]) ?></td>
    <td style="border: 1px solid black; width: 106px; <?php if(is_null($produit["sgn"])): ?>background-color: #bbb;<?php endif; ?>"><?php echoVolume($produit["sgn"]) ?></td>
  </tr>
<?php endforeach; ?>
<tr>
