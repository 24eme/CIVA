function(doc) {
 ...
 if(appellation_key.match('^appellation')) {
          ...
          if(couleur_key.match('^couleur')) {
           for(cepage_key in doc.recolte[appellation_key][lieu_key][couleur_key]) {
              if(cepage_key.match('^cepage')) {
               emit([doc.campagne, doc.declaration_insee, appellation_key, cepage_key], 
                     {
   ...
   emit([doc.campagne, doc.declaration_insee,  appellation_key, null], 
        {
   ...
   emit([doc.campagne, doc.declaration_insee,  null, null], 
        {
         "nb_declarations": 1  
        });
