...
class importConfiguration2014Task extends sfBaseTask
{
...
EOF;
    ...
    protected function execute($arguments = array(), $options = array())
    {
        ...
        $rendement_couleur_blanc_cremant = 82;
        $rendement_couleur_rose_cremant = 82;
        ...
        $json->recolte->certification->genre->appellation_ALSACEBLANC->rendement_appellation = 82;
        ...
        $cepcom = array();
        foreach (file(sfConfig::get('sf_data_dir') . '/import/'.substr($annee, 2, 2).'/Cepcom'.substr($annee, 2, 2)) as $l) {
        ...
        $appellation->mention->lieuBERG->libelle = "Bergheim";
        $appellation->mention->lieuBERG->couleurBlanc->douane->appellation_lieu = $cepcom['BERG']['GW'];
        $appellation->mention->lieuBERG->couleurBlanc->douane->couleur = 'B';
        $appellation->mention->lieuBERG->couleurBlanc->rendement_couleur = $rendement_couleur_blanc_communale;
        $appellation->mention->lieuBERG->couleurBlanc->libelle = "Blanc";
        $appellation->mention->lieuBERG->couleurBlanc->cepage_GW = $this->getCepage('GW', $cepage_douane[7]['GW'], 'S0');
        ...
        $appellation->mention->lieuCOHK->libelle = "Coteaux du Haut-Koenigsbourg";
        $appellation->mention->lieuCOHK->couleurBlanc->douane->appellation_lieu = $cepcom['COHK']['AL'];
        $appellation->mention->lieuCOHK->couleurBlanc->douane->couleur = 'B';
        $appellation->mention->lieuCOHK->couleurBlanc->rendement_couleur = $rendement_couleur_blanc_communale;
        $appellation->mention->lieuCOHK->couleurBlanc->libelle = "Blanc";
        $appellation->mention->lieuCOHK->couleurBlanc->cepage_RI = $this->getCepage('RI', $cepage_douane[7]['RI'], 'S0');
        $appellation->mention->lieuCOHK->couleurBlanc->cepage_GW = $this->getCepage('GW', $cepage_douane[7]['GW'], 'S0');
        ...
        foreach (file(sfConfig::get('sf_data_dir') . '/import/'.substr($annee, 2, 2).'/Grdcrv'.substr($annee, 2, 2)) as $l) {
        ...
        $json->recolte->certification->genre->appellation_CREMANT->rendement_appellation = 82;
        ...
        $json->recolte->certification->genre->appellation_CREMANT->mention->lieu->couleur->cepage_BL->douane->couleur = 'B';
        $json->recolte->certification->genre->appellation_CREMANT->mention->lieu->couleur->cepage_BL->douane->qualite = 'M ';        
    ...
    public function getCepage($code, $code_depage = null, $qualite = null, $rendement = null)
    {
        ...
        $cepages->cepage_RI->rendement = 85;
