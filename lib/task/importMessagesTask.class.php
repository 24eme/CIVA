...
EOF;
	...
	$json->err_dr_popup_min_quantite = "Les rebêches doivent représenter au minimum 2 % du volume total produit";
        ...
        $json->help_popup_DR_dplc = "Nous vous indiquons ici votre dplc en fonction de ce que vous avez saisi. Nous vous rappelons que dans l'AOC Alsace, en cas de dépassement d'un butoir cépage, c'est le DPLC de ce cépage qu'il faudra obligatoirement livrer en distillerie même si vous avez moins (ou pas du tout) de DPLC sur le total de l'appellation. Voir les exemples dans la notice explicative générale que vous pouvez consulter (et imprimer) dans l'écran « mon espace CIVA ».";
	$json->help_popup_DR_total_cepage = "Nous vous indiquons ici, en fonction de ce que vous avez saisi, la répartition de votre rendement dans le total cépage (volume revendiqué et DPLC éventuel). Dans l'AOC Alsace, si le rendement à l'ha apparaît en rouge c'est que vous dépassez le rendement maximum butoir de ce cépage.";
