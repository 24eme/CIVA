...
class ExportDRAcheteursCsv {
    ...
    protected function load($campagne) {
                ...
                foreach ($dr->recolte->appellations as $appellation) {
                    foreach ($appellation->getLieux() as $lieu) {
                        ...
                        foreach ($lieu->acheteurs as $acheteurs) {
                            foreach ($acheteurs as $cvi_acheteur => $acheteur) {
                                $this->addTotal($cvi_acheteur, $acheteur);
                            }
                        }
                    }
                }
    ...
    protected function add($cvi, $datas, $validation) {
        if (!array_key_exists($cvi, $this->_exports)) {
            $this->_exports[$cvi] = new ExportCsv();
        }
    ...
    public function getOutputs() {
        $outputs = array();
        foreach($this->_exports as $export) {
